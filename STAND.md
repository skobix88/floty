...
**Stand: 30.08.2026 — M1 bis M5 gebaut, 124 Prüfungen laufen durch, 1.0.0-rc.1 gepackt.**
...
### M5 — Zwischenablage-Verlauf · gebaut

| Baustein | Datei | Stand |
|---|---|---|
| Eintrag, Namen, Fingerabdruck | `Clipboard/ClipboardEntry.swift` | automatisiert geprüft |
| Liste: Reihenfolge, Dubletten, Grenzen, Suche | `Clipboard/ClipboardHistory.swift` | automatisiert geprüft |
| Was aufgenommen werden darf | `Clipboard/PasteboardPolicy.swift` | automatisiert geprüft **und in der laufenden App bestätigt** |
| Ablage, Bilder, Miniaturen | `Clipboard/ClipboardStore.swift` | automatisiert geprüft |
| Abfrageschleife | `Clipboard/ClipboardWatcher.swift` | in der laufenden App bestätigt |
| Auswahlfenster, Tastaturbedienung | `Clipboard/ClipboardWindow.swift`, `ClipboardListView.swift` | gebaut, nur kompiliert |
| Zeitangabe | `Clipboard/ClipboardTimestamp.swift` | automatisiert geprüft |
| Einstellungsabschnitt inkl. Hinweis beim Einschalten | `Settings/SettingsView.swift` | gebaut, nur kompiliert |

**In der laufenden App nachgestellt** (Fassung aus `/Applications`, Funktion
eingeschaltet, danach wieder ausgeschaltet und Testdaten entfernt):

1. Text kopiert → Eintrag angelegt, Name ist die erste Zeile
2. Ein als `org.nspasteboard.ConcealedType` markiertes Passwort kopiert →
   **nicht** im Verlauf
3. Bild kopiert → Eintrag plus Vollbild und Miniatur auf der Platte
4. Denselben Text erneut kopiert → wandert nach oben, keine Dublette
5. Ausgeschaltet entsteht kein Ordner in Application Support

**Noch nicht nachgestellt:** die Bedienung des Fensters — Suchen, Pfeiltasten,
Enter, Esc, ⌘⌫ — und der Einstellungsabschnitt.
...
protokolliert alles, was durchs System läuft. Deshalb ein eigenes Fenster mit
eigenem Kurzbefehl statt eines weiteren Reiters, und deshalb **standardmäßig
aus**. Wer es nicht will, merkt nicht, dass es da ist: kein Timer, kein
Menüeintrag, kein Ordner in Application Support.

Der Ausschlag gab der Arbeitsfluss: den Verlauf braucht man, *während* man in
einer anderen App ist. Flotys Panel blendet sich beim Klick außerhalb aus — eine
Historie, die genau dann verschwindet, wenn man sie braucht, wäre wertlos.

### Passwörter sind der schwierige Teil, nicht die Liste

Eine mitschreibende Zwischenablage fängt jedes kopierte Passwort ab. Die
Konvention von nspasteboard.org (`ConcealedType`, `TransientType`,
`AutoGeneratedType`) ist deshalb **fest verdrahtet**, keine Einstellung — auf
diesem Rechner sind 1Password und Bitwarden installiert, beide halten sich daran.
In der laufenden App nachgestellt.

Die Ausschlussliste für Programme ist die zweite Ebene, aber bewusst zurückhaltend
beschrieben: macOS verrät nicht, wer etwas in die Zwischenablage gelegt hat.
Genommen wird das vorderste Programm im Moment der Erkennung — fast immer
richtig, aber kein Sicherheitsversprechen.

Bleibendes Restrisiko, offen benannt: ein Passwort aus einer E-Mail trägt keine
Markierung und läge unverschlüsselt im Benutzerordner. Dagegen helfen nur
„Pausieren" und „Verlauf leeren" — und der Hinweis beim ersten Einschalten.

### Rückkopplung verhindern

Legt Floty selbst etwas in die Zwischenablage, merkt sich der Wächter den daraus
entstehenden `changeCount` und überspringt ihn. Ohne das wanderte jeder Eintrag
bei jeder Benutzung nach oben und die Reihenfolge wäre wertlos.

### Zwei Grenzen statt einer

Eine Anzahl allein reicht nicht, sobald Bilder mitlaufen: fünfzig Bildschirmfotos
sind mehrere hundert Megabyte. Es gilt, was zuerst greift — Anzahl oder
Gesamtgröße. Dazu ein Deckel je Objekt, damit ein einzelnes riesiges Bild gar
nicht erst aufgenommen wird.

### Zeitangabe gegen den Bezugszeitpunkt, nicht gegen den Kalendertag

Erster Anlauf benutzte `Calendar.isDateInToday`. Das vergleicht mit dem echten
heutigen Tag statt mit dem übergebenen Bezugszeitpunkt — die Angabe wäre falsch,
sobald jemand über Mitternacht hinweg arbeitet. Ein Test hat es gefunden, bevor
...
**124 Prüfungen in 22 Gruppen, alle grün** (`xcodebuild test`, Swift Testing).
...
| Zwischenablage: Namen | erste sinnvolle Zeile, Leerraum, Kürzung, leerer Text, Bildmaße, Fingerabdrücke |
| Zwischenablage: Verlauf | Reihenfolge, Dubletten, Anzahl, Gesamtgröße, zu großes Objekt, Löschen, Leeren, Suche |
| Zwischenablage: Aufnahme | gewöhnlicher Text, alle Verborgen-Markierungen, Ausschlussliste, unbekannte Quelle |
| Zwischenablage: Ablage | Testordner-Sperre, Schreiben/Einlesen, kaputter Index, Bild und Miniatur, Löschen nimmt Dateien mit, Leeren, belegter Platz, kein Ordner im Leerlauf |
| Zwischenablage: Zeitangabe | gerade eben, heute, gestern, älter |
| Auswahlfenster | waagerecht mittig, oberes Drittel, hohes Fenster passt |
